...
export abstract class Joinable extends SqlFrom{
	...
	public innerJoin(other: Joinable, joinOn?: [QueryColumn, QueryColumn]): Joinable{
		return this.genericJoin(other, "INNER", joinOn);
	}

	public leftJoin(other: Joinable, joinOn?: [QueryColumn, QueryColumn]): Joinable{
		return this.genericJoin(other, "LEFT", joinOn);
	}

	public rightJoin(other: Joinable, joinOn?: [QueryColumn, QueryColumn]): Joinable{
	...
	private genericJoin(other: Joinable, joinType: "INNER" | "LEFT" | "RIGHT", joinOn?: [QueryColumn, QueryColumn]): Joinable{
		let joinOnStr: [string, string];
		if(!joinOn){
			let ambiguousError = `Ambiguous join between "${this.__sqlFrom}" and "${other.__sqlFrom}"`;
			let joinOn1 = this.findJoinableKeys(this.__primaryKeys, other.__foreignKeys, ambiguousError);
			let joinOn2 = this.findJoinableKeys(other.__primaryKeys, this.__foreignKeys, ambiguousError);
	...
	private findJoinableKeys(pks: JoinablePrimaryKey[], fks: ForeignKey[], error: string): [string, string] | null {
...
export abstract class Table extends Joinable{
	...
	protected get __sqlFrom(): string {
		return this.__tableName == this.__alias ? this.__tableName : (this.__tableName + " AS " + this.__alias);
	}
}